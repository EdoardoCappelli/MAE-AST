...
def print_header(dataset_name="Combined Pretraining", use_validation=True, device='cpu', resume_info=None):
    print("\n" + "="*80)
    header_text = f'MAE Pre-training on {dataset_name} using {device}'
...
def train(args, train_loader, model, optimizer, epoch, total_epochs, print_freq=100, device='cuda', use_wandb=False, initial_step=0):
    ...
    for i, batch_data in enumerate(train_loader): 
        ...
        # Forward pass per MAE
        target, recon_logits, class_logits, bool_mask = model(input_waveform)
        
        recon_loss = mae_loss(target, recon_logits) * 10 
        class_loss = infoNCE_loss(target, class_logits) * 1 
        loss = recon_loss + class_loss 
        
        recon_losses.update(recon_loss.item(), input_waveform.size(0))
        class_losses.update(class_loss.item(), input_waveform.size(0))
...
def validate(args, val_loader, model, epoch, print_freq=100, device='cuda'):
    batch_time = AverageMeter()
    losses = AverageMeter()
    recon_losses = AverageMeter()
    class_losses = AverageMeter()
...
def main():   
    ...
    if args.masking_strategy == 'patch':
        random = False # uso approccio chunked 
    elif args.masking_strategy == 'frame':
        random = True
    else:
        raise ValueError("masking_strategy deve essere 'patch' o 'frame'")

    config = Config()
    checkpoint_dir = config.pretraining_checkpoints_dir + '_' + args.masking_strategy + '/librispeech&audioset'
    os.makedirs(checkpoint_dir, exist_ok=True)

    # Istanzia il modello MAE
    model = MAE(args, config, pretraining=True)
    ...
    if config.use_wandb:
        wandb.init(
            project=config.pretraining_wandb_project + f"-{args.masking_strategy}",
        ...
        run_name = f'pretrain-{config.pretraining_wandb_project}-{args.masking_strategy}-epochs_{args.epochs}-lr_{config.initial_lr}'
        if resume_info:
    ...
    print("\n=== Inizializzazione Dataloader per il Pre-training ===")
    ...
    librispeech_base_dataset = None
    audioset_base_dataset = None

    try:
        librispeech_base_dataset = LibriSpeechAudioDataset(
        ...
        print(f"Avviso: Impossibile caricare il dataset LibriSpeech: {e}")

    try:
        audioset_base_dataset = AudioSetAudioDataset( 
            root=config.audioset_root,
            transform=spectrogram_transform
        )
    except RuntimeError as e:
        print(f"Avviso: Impossibile caricare il dataset AudioSet: {e}") 

    pretraining_combined_dataset = None

    if librispeech_base_dataset or audioset_base_dataset:
        try:
            pretraining_combined_dataset = PretrainingDataset(
                librispeech_dataset=librispeech_base_dataset,
                audioset_dataset=audioset_base_dataset, 
                librispeech_percentage=config.librispeech_percentage,
                audioset_percentage=config.audioset_percentage 
            )
        except RuntimeError as e:
            print(f"Errore nella creazione del PretrainingDataset combinato: {e}. Il training non verrà avviato.")
            return 
    else:
        print("Nessun dataset di base (LibriSpeech o AudioSet) è stato caricato con successo. Impossibile procedere con il pre-training. Uscita.")
        return 

    if config.use_validation:
        dataset_size = len(pretraining_combined_dataset)
        val_size = int(config.validation_split_ratio * dataset_size)
        train_size = dataset_size - val_size
        
        train_dataset, val_dataset = torch.utils.data.random_split(
            pretraining_combined_dataset, [train_size, val_size], generator=torch.Generator().manual_seed(42) 
        )
        print(f"Dataset combinato suddiviso: {len(train_dataset)} campioni per il training, {len(val_dataset)} per la validazione.")
    else:
        train_dataset = pretraining_combined_dataset
        val_dataset = None
        print("Validation disabilitata. L'intero dataset combinato verrà usato per il training.")
    ...
    print_header("Combined Pretraining Dataset (LibriSpeech + AudioSet)", config.use_validation, device, resume_info)
    ...
    checkpoint_epochs = config.pretraining_checkpoint_epochs
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        train_loss, train_recon_loss, train_class_loss, iters_in_epoch = train(
            args, train_loader, model, optimizer, epoch, total_epochs, 
        ...
        if config.use_validation and val_loader is not None:
            val_loss, val_recon_loss, val_class_loss = validate(args, val_loader, model, epoch, config.print_freq, device)
    ...
    plot_normalized_loss(args, all_iterations, all_train_losses, train=True, save_path=f'./plots/pretraining_loss_train_{args.masking_strategy}.png')  
    plot_normalized_loss(args, all_iterations, all_val_losses, train=False, save_path=f'./plots/pretraining_loss_val_{args.masking_strategy}.png') if config.use_validation else None
