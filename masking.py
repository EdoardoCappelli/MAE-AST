...
class Mask(nn.Module):
    def __init__(self, config):
        ...
        # Parametri per chunked masking
        self.chunk_size_range = getattr(config, 'chunk_size_range', (3, 5))  # (min, max)
        self.patches_per_row = getattr(config, 'img_size')[0] // getattr(config, 'patch_size')[0] 
        self.chan_adjust = getattr(config, 'chan_adjust', 1)  # Equivalente al chan_adjust del codice originale
    ...
    def _chunk_masking(self, B: int, num_patches: int, total_patches_to_mask: int, device: torch.device) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
        """
        Implementa il chunked masking seguendo la logica di SSAST.
        Crea chunk rettangolari partendo da posizioni casuali.
        """
        masked_indices_list = []
        unmasked_indices_list = []
        
        # Genera un singolo pattern di masking per tutto il batch (come nel codice originale)
        cur_masked_idx = set()
        chunk_size = random.randrange(self.chunk_size_range[0], self.chunk_size_range[1] + 1)
        
        # Se non abbiamo informazioni sulla disposizione 2D, assumiamo una disposizione lineare
        if self.patches_per_row is None:
            # Disposizione lineare - simula il comportamento temporale
            chan_adjust = self.chan_adjust
        else:
            # Disposizione 2D reale
            chan_adjust = self.patches_per_row
        
        # Continua ad aggiungere chunk finché non abbiamo abbastanza patch mascherate
        while len(cur_masked_idx) < total_patches_to_mask:
            # Seleziona posizione casuale di partenza
            t_topleft = random.randrange(num_patches)
            
            # Crea chunk rettangolare
            for t_offset in range(chunk_size):  # Offset temporale/verticale
                for c_offset in range(chunk_size):  # Offset dei canali/orizzontale
                    mask_cand = t_topleft + t_offset + chan_adjust * c_offset
                    if mask_cand < num_patches:
                        cur_masked_idx.add(mask_cand)
        
        # Converti in lista e tronca al numero desiderato
        cur_masked_idx = list(cur_masked_idx)[:total_patches_to_mask]
        cur_retained_idx = list(set(range(num_patches)).difference(cur_masked_idx))
        
        # Converti in tensori per ogni elemento del batch
        masked_tensor = torch.tensor(cur_masked_idx, device=device)
        unmasked_tensor = torch.tensor(cur_retained_idx, device=device)
        
        # Usa lo stesso pattern per tutto il batch (come nel codice originale)
        for i in range(B):
            masked_indices_list.append(masked_tensor.clone())
            unmasked_indices_list.append(unmasked_tensor.clone())
    ...
    def forward(self, patch_embeddings: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, List[torch.Tensor], List[torch.Tensor]]:
        ...
        # Selezione della strategia di masking
        if self.masking_strategy == 'chunk':
            masked_indices_list, unmasked_indices_list = self._chunk_masking(
                B, num_patches, total_patches_to_mask, device
            )
        else:  # 'random' o qualsiasi altra strategia
        ...
        for b in range(B):
            bool_mask[b, masked_indices_list[b]] = True
        ...
        # Estrazione delle patch non mascherate
        batch_indices = torch.arange(B, device=device).unsqueeze(1)
        
        # Per il chunked masking, dobbiamo gestire tensori di diverse lunghezze
        max_unmasked = max(len(unmasked_indices_list[i]) for i in range(B))
        unmasked_patches_only = torch.zeros(B, max_unmasked, patch_embedding_dim, device=device)
        
        for b in range(B):
            unmasked_len = len(unmasked_indices_list[b])
            unmasked_patches_only[b, :unmasked_len] = patch_embeddings[b, unmasked_indices_list[b]]
        
        return (unmasked_patches_only, 
                bool_mask, 
                masked_indices_list, 
                unmasked_indices_list)


# Esempio di configurazione per abilitare il chunked masking
class ConfigExample:
    def __init__(self):
        self.enc_embed_dim = 768
        self.masking_percentage = 0.75
        self.masking_strategy = 'chunk'  # 'chunk' o 'random'
        self.patch_size = [16, 16]  # Assumendo patch 16x16
        
        # Parametri specifici per chunked masking
        self.chunk_size_range = (3, 5)  # Range dimensioni chunk
        self.patches_per_row = 14  # Numero di patch per riga (per immagini 224x224 con patch 16x16)
        self.chan_adjust = 14  # Stesso valore di patches_per_row per disposizione 2D standard


# Esempio di utilizzo
if __name__ == "__main__":
    config = ConfigExample()
    mask_module = Mask(config)
    
    # Simula patch embeddings (8 batch, 196 patch, 768 dimensioni)
    patch_embeddings = torch.randn(8, 196, 768)
    ...
    (masked_embeddings, 
     ...
     bool_mask, 
     masked_idx, 
     unmasked_idx) = mask_module(patch_embeddings)
    
    print(f"Shape embeddings mascherati: {masked_embeddings.shape}")
    print(f"Shape embeddings non mascherati: {unmasked_only.shape}")
    print(f"Numero patch mascherate per batch: {[len(idx) for idx in masked_idx]}")
    print(f"Numero patch non mascherate per batch: {[len(idx) for idx in unmasked_idx]}")
    
    # Verifica che il chunked masking crei pattern contigui
    if config.masking_strategy == 'chunk':
        print("Primi 20 indici mascherati del primo batch:", masked_idx[0][:20].tolist())
# class Mask(nn.Module):
...
#         # Extract only unmasked patches for the encoder (it encoder only sees unmasked patches)
