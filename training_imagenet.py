...
# from timm_implementation import MAE   
from transformer import MAE
# --------------------------------------------------
# DATA LOADER
# --------------------------------------------------
def data_loader(root_train, root_val, batch_size=256, workers=4, pin_memory=True):
    """
...
# TRAIN FUNCTION
# --------------------------------------------------
def train(train_loader, model, criterion, optimizer, epoch, print_freq=100, device='cuda'):
    """
    ...
    for i, (input, _) in enumerate(train_loader):
        ...
        # Forward
        output = model(input)
        # Il modello MAE dovrebbe tornare un dict con almeno:
        #   output["recon_patches"]  → tensor [B, ..., patch_dim]
        #   output["target_patches"] → tensor [B, ..., patch_dim]
        recon = output["recon_logits"]
        target = output["target_patches"]
...
def main():
    ...
    config = SimpleNamespace(
        ...
        num_enc_hidden_layers = 6,
        num_enc_attention_heads = 12,
    ...
    train_loader, val_loader = data_loader(
        ...
        batch_size=config.batch_size, workers=4, pin_memory=True
    )
    ...
    for epoch in range(config.epochs):
        ...
        avg_train_loss = train(train_loader, model, criterion, optimizer,
                               epoch, print_freq=config.print_freq, device=device)

        # VALIDATE
        avg_val_loss = validate(val_loader, model, criterion,
                                print_freq=config.print_freq, device=device)

        # Salvo checkpoint se è il migliore finora
